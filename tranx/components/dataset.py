...
class Dataset(object):
    ...
    def batch_iter(self, batch_size, shuffle=False,index_arr=None):
        if shuffle and not index_arr:
            index_arr = self.shuffle_data()
...
class Batch(object):
    def __init__(self, examples, grammar, vocab, copy=True, cuda=False):
        ...
        self.copy = copy
        self.cuda = cuda
        self.tgt_lens = [len(e.tgt_actions) for e in self.examples]
