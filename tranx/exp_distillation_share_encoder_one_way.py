...
def train(args):
    ...
    model_student.level_traversal = (args.traverse_first != "deep")
    model_teacher.level_traversal = not model_student.level_traversal
    ...
    while True:
        ...
        for (batch_student_examples,batch_teacher_examples) in zip(train_student_set.batch_iter(batch_size=args.batch_size, index_arr=index_arr), train_teacher_set.batch_iter(batch_size=args.batch_size, index_arr=index_arr)):
            ...
            if epoch <= args.pretrain_epoch:
                ret_student_val, KL, batch_len = model_student.score(batch_student_examples,[q.detach() for q in teacher_hidden],transformList)
        ...
        if (patience_student >= args.patience) and epoch >= args.lr_decay_after_epoch:
            num_trial += 1
