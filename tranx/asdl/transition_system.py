...
class TransitionSystem(object):
    ...
    def get_actions(self, asdl_ast,level_traversal=False,right_left=False):
        """
        ...
        if level_traversal == False:
            ...
            for field in (asdl_ast.fields[::-1] if right_left else asdl_ast.fields):
                # is a composite field
                if self.grammar.is_composite_type(field.type):
                    if field.cardinality == 'single':
                        field_actions = self.get_actions(field.value,right_left=right_left)
                    else:
                        field_actions = []
                        ...
                        if field.value is not None:
                            if field.cardinality == 'multiple':
                                ...
                                #    field_names = field.value[::-1]
                                    #field_names = field.value[:-1][::-1]
                                    #field_names.append(field.value[-1]) 
                                    #print("field.value:",field.value)
                                
                                for val in field_names:
                                    cur_child_actions = self.get_actions(val,right_left=right_left)
                    ...
                    #isClassName = False
                    #if (parent_action.production.constructor.name[:8]=="ClassDef"):
                    #    isClassName = True
                    #field_actions = self.get_primitive_field_actions(field,isClassName)
            ...
            stack = [tuple([asdl_ast])]
            while(len(stack) >= 1):
                top = stack.pop(0)
                if isinstance(top, tuple): 
                    actions.append(ApplyRuleAction(top[0].production))
                    top = top[0]
                else:
                    actions.append(top)
                    continue
                ...
                for field in top.fields:
                    if self.grammar.is_composite_type(field.type):
                        if field.cardinality == 'single':
                            field_actions = [tuple([field.value])]
                            ...
                            if field.value is not None:
                                if field.cardinality == 'multiple':
                                    for val in field.value:
                                        cur_child_actions = tuple([val])
                                        field_actions.append(cur_child_actions)
                        ...
                        #if (top.production.constructor.name[:8]=="ClassDef"):
                        #    isClassName = True
                        #field_actions = self.get_primitive_field_actions(field,isClassName)
                        field_actions = self.get_primitive_field_actions(field)
                        ...
                        if field.cardinality == 'multiple' or field.cardinality == 'optional' and not field_actions:
                            # reduce action
                            field_actions.append(ReduceAction())
                    stack.extend(field_actions)
                
                
        #actions.reverse() if right_left else None
        return actions
