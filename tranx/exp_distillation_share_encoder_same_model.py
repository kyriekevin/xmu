...
def train(args):
    ...
    if args.pretrain:
        ...
        src_embed = model_teacher.src_embed = model_student.src_embed 
        model_teacher.encoder_lstm = model_student.encoder_lstm 
    else:
        ...
        model_student = parser_cls_student(args, vocab, transition_system, init=True,srcEmbed=src_embed,srcLstm=encoder_lstm)
        model_teacher = parser_cls_teacher(args, vocab, transition_system, init=True,srcEmbed=src_embed,srcLstm=encoder_lstm)
        
    if args.traverse_first == "layer" :
        model_student.level_traversal = True
        model_teacher.level_traversal = True
    else:
    ...
    #nn_utils.uniform_init(-args.uniform_init, args.uniform_init, model_student.parameters()) if args.uniform_init else None
    nn_utils.glorot_init(model_student.parameters(),gain=args.gain_init) if args.gain_init else None
    ...
    while True:
        ...
        for (batch_student_examples,batch_teacher_examples) in zip(train_student_set.batch_iter(batch_size=args.batch_size, index_arr=index_arr), train_teacher_set.batch_iter(batch_size=args.batch_size, index_arr=index_arr)):
            ...
            if train_iter % args.log_every == 0:
                log_student_str = '[Iter %d] encoder student loss=%.5f ( lambda=%f)' % (train_iter, report_student_loss / report_student_examples,args.lambda_num)
        ...
        if args.dev_file:
            ...
            if epoch % args.valid_every_epoch == 0 and epoch >= args.vaildate_begin_epoch:
                ...
                is_better = history_dev_scores == [] or (dev_score +dev_score_student)/2 >= max(history_dev_scores)
                history_dev_scores.append((dev_score +dev_score_student)/2)
                history_dev_scores.sort(reverse=True)
                
                
                print('[Epoch %d] evaluate details: dev %s: %.5f (took %ds)' % (
                                    epoch, 
                                    evaluator_teacher.default_metric,
                                    (dev_score +dev_score_student)/2,
                ...
                if args.test_file:
                    test_results = evaluation.evaluate(test_set.examples, model_student, evaluator_student, args
                                                       , eval_top_pred_only=args.eval_top_pred_only)
                    test_score1 = test_results[evaluator_student.default_metric]
                               
                        
                    test_results = evaluation.evaluate(test_set.examples, model_teacher, evaluator_teacher, args
                                                       , eval_top_pred_only=args.eval_top_pred_only)
                    test_score2 = test_results[evaluator_teacher.default_metric]
                    print('[Epoch %d] test details: dev %s: %.5f ' % (epoch,
                                                             evaluator_teacher.default_metric,
                                                            (test_score1+test_score2)/2))
...
def test(args):
    ...
    params = torch.load(args.load_model+".teacher.bin", map_location=lambda storage, loc: storage)
    ...
    parser_teacher.eval()
    eval_results_teacher = evaluation.evaluate(test_set.examples, parser_teacher, evaluator, args,
    ...
    parser_student.eval()
    eval_results_student= evaluation.evaluate(test_set.examples, parser_student, evaluator, args,
                                                       verbose=args.verbose)    
    eval_results = {}
    for i in eval_results_student:
        eval_results[i] = (eval_results_student[i] + eval_results_teacher[i]) /  2
