...
def train(args):
    ...
    while True:
        ...
        # perform validation
        is_better = False
        if args.dev_file and epoch >= args.vaildate_begin_epoch:
            if epoch % args.valid_every_epoch == 0 :
                ...
                if args.test_file:
                    ...
                    test_results = evaluation.evaluate(test_set.examples, model, evaluator, args
                                                       , eval_top_pred_only=args.eval_top_pred_only)
                    test_score = test_results[evaluator.default_metric]

                    print('[Epoch %d] test details: %s, test %s: %.5f (took %ds)' % (
                                        epoch, test_results,
                                        evaluator.default_metric,
                                        test_score,
                                        time.time() - test_start), file=sys.stderr)
        else:
            is_better = True
...
def test(args):
    ...
    parser.level_traversal = (args.traverse_first != "deep")
    parser.right_left = args.right_left
    parser.args = saved_args
    parser.eval()
    evaluator = Registrable.by_name(args.evaluator)(transition_system, args=args)
...
def result_eval(args):
    ...
    teacher_name = args.load_model + ".teacher.bin"
    student_name = args.load_model + ".student.bin"
    
    parser_teacher = parser_cls.load(model_path=teacher_name , cuda=args.cuda)
    parser_teacher.level_traversal = True
    parser_teacher.right_left = False
    parser_teacher.args = saved_args
    parser_teacher.eval()
    
    
    parser_student = parser_cls.load(model_path=student_name, cuda=args.cuda)
    parser_student.level_traversal = False
    parser_student.right_left = False
    parser_student.args = saved_args
    parser_student.eval()
    
    
    model_name2= "saved_models/django/model.django.seed0.dropout0.4.share.encoder.mutual.pretrain.lambda0.0"
    #"saved_models/django/model.django.seed1.pretain.lambda0.5.dropout0.4.share.encoder"
    teacher_name = model_name2 + ".teacher.bin"
    student_name = model_name2 + ".student.bin"
    
    parser_teacher2 = parser_cls.load(model_path=teacher_name , cuda=args.cuda)
    parser_teacher2.level_traversal = True
    parser_teacher2.right_left = False
    parser_teacher2.args = saved_args
    parser_teacher2.eval()
    
    
    parser_student2 = parser_cls.load(model_path=student_name, cuda=args.cuda)
    parser_student2.level_traversal = False
    parser_student2.right_left = False
    parser_student2.args = saved_args
    parser_student2.eval()    
    ...
    for data_file in test_data:
        ...
        eval_results, decode_results_teacher = evaluation.evaluate(test_set.examples,  parser_teacher, evaluator, args, verbose=args.verbose, return_decode_result=True)
        eval_results, decode_results_student = evaluation.evaluate(test_set.examples,  parser_student, evaluator, args, verbose=args.verbose, return_decode_result=True)  
        
        eval_results, decode_resultsBiDecoder = evaluation.evaluateBiDecoder(test_set.examples, parser_student, parser_teacher, evaluator, args,
verbose=args.verbose, return_decode_result=True)        
        
        
        
        eval_results, decode_results_student2 = evaluation.evaluate(test_set.examples,  parser_student2, evaluator, args, verbose=args.verbose, return_decode_result=True)           
        
        f_num = open("num.log", 'w')
        f_num.close()
        f_wrong = open("wrong.log", 'w')
        f_wrong.close()
        for exe_student, exe_teacher, exe_BiDecoder, exe_student2, fair_exe in zip(decode_results_student, decode_results_teacher, decode_resultsBiDecoder, decode_results_student2,  test_set.examples) :
            index = 0
            if len(exe_student) >0 and len(exe_teacher) >0 and len(exe_BiDecoder) >0 and len(exe_student2) >0:
                current_student = exe_student[0]
                current_teacher = exe_teacher[0]
                current_BiDecoder = exe_BiDecoder[0]
                current_student2 = exe_student2[0]
                
                if not current_student.is_correct and not current_teacher.is_correct and current_student2.is_correct and not current_BiDecoder.is_correct: 
                    print('%d' % (index), file=open("num.log","a+"))
                    print("src_sent: %s " % (" ".join(fair_exe.src_sent)) , file=open("wrong.log","a+"))
                    print("tgt_code: %s " % (fair_exe.tgt_code), file=open("wrong.log","a+"))
                    print("translate_code_student: %s" % current_student.code, file=open("wrong.log","a+"))
                  
                    print("translate_code_teacher: %s" % current_teacher.code, file=open("wrong.log","a+"))
                    
                    print("translate_code_BiDecoder: %s" % current_BiDecoder.code, file=open("wrong.log","a+"))
                  
                    print("translate_code_student_base: %s" % current_student2.code, file=open("wrong.log","a+"))
                   
                    print("**********************", file=open("wrong.log","a+"))
            index += 1
        result_dict[data_file] = eval_results
    ...
    for data_file in test_data:
        ...
        #eval_results, decode_resultsBiDecoder = evaluation.evaluateBiDecoder(test_set.examples, parser_student, parser_teacher, evaluator, args,verbose=args.verbose, return_decode_result=True)        
        
        
        
        eval_results, decode_results_student2 = evaluation.evaluate(test_set.examples,  parser_student, evaluator, args, verbose=args.verbose, return_decode_result=True)           
        
        f_num = open("num.log", 'w')
        f_num.close()
        f_wrong = open("wrong_specific.log", 'w')
        f_wrong.close()
        for exe_student2, fair_exe in zip(decode_results_student2, test_set.examples) :
            index = 0
            if len(exe_student2) >0 and " ".join(fair_exe.src_sent) in ["call the method line.lstrip [ line . lstrip ] , if the result starts with TRANSLATOR_COMMENT_MARK ,"]:
                for nums in range(len(exe_student2)):
                    current_student2 = exe_student2[nums]
                    print("translate_code_student_base: %s" % current_student2.code, file=open("wrong_specific.log","a+"))
                   
                    print("**********************", file=open("wrong_specific.log","a+"))
            index += 1
        result_dict[data_file] = eval_results        
