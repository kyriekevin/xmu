...
@Registrable.register('default_student_parser')
class ParserStudent(nn.Module):
    ...
    def __init__(self, args, vocab, transition_system,init=True,srcEmbed=None,srcLstm=None):
        ...
        if args.func == "KL":
            self.criterion_kl_loss = torch.nn.KLDivLoss(reduction='none')
        else:
            self.distillation_in = nn.Linear(args.hidden_size,args.hidden_size, bias=True)
            self.distillation_out = nn.Linear(args.hidden_size,args.hidden_size, bias=True)
        ...
        if args.lstm == 'lstm':
            ...
            # frontier info
            input_dim =input_dim + args.action_embed_size * (not args.no_parent_production_embed)
            input_dim =input_dim + args.field_embed_size * (not args.no_parent_field_embed)
            input_dim =input_dim + args.type_embed_size * (not args.no_parent_field_type_embed)
            input_dim =input_dim + args.hidden_size * (not args.no_parent_state)

            input_dim =input_dim + args.att_vec_size * (not args.no_input_feed)  # input feeding
        ...
        self.src_dropout = nn.Dropout(args.src_dropout)
    ...
    def encode(self, src_sents_var, src_sents_len):
        ...
        src_token_embed = self.src_dropout(src_token_embed)
    ...
    def transform_match(self,out_vocab_vectors,teacher_hidden,transformList,batch):
        from torch.autograd import Variable
        transformList = Variable(torch.cuda.LongTensor(transformList))
        student_hidden_transforms = None
        teacher_hidden_transforms = None
        for i in range(transformList.shape[0]):
            
            student_hidden_transforms_1 = out_vocab_vectors.transpose(0, 1)[i]
            teacher_hidden_transforms_1 = teacher_hidden.transpose(0, 1)[i][transformList[i]]

            if teacher_hidden_transforms is None:
                #student_hidden_transforms_1[len(batch.action_list[i]):] = 0
                #teacher_hidden_transforms_1[len(batch.action_list[i]):] = 0
                student_hidden_transforms = student_hidden_transforms_1.unsqueeze(-1)
                teacher_hidden_transforms = teacher_hidden_transforms_1.unsqueeze(-1)


            else:
                #student_hidden_transforms_1[len(batch.action_list[i]):] = 0
                #teacher_hidden_transforms_1[len(batch.action_list[i]):] = 0
                student_hidden_transforms = torch.cat( (student_hidden_transforms ,
                                    student_hidden_transforms_1.unsqueeze(-1)),-1 )
                teacher_hidden_transforms = torch.cat( (teacher_hidden_transforms ,
                                    teacher_hidden_transforms_1.unsqueeze(-1)),-1 )
        #print("student_hidden_transforms_1:",student_hidden_transforms_1.shape)
        return student_hidden_transforms,teacher_hidden_transforms
    ...
    def score(self, examples,teacher_hidden,transformList,return_encode_state=False):
        ...
        out_vocab_vectors = self.tgt_token_readout(query_vectors)

        delta_hidden = None

        if transformList is not None:
            if self.args.func == "OLSE":
                student_hidden_transforms = query_vectors
                teacher_hidden_transforms = teacher_hidden
                teacher_hidden_transforms = teacher_hidden_transforms.transpose(0,2)
                teacher_hidden_transforms = teacher_hidden_transforms.transpose(1,2)
                student_hidden_transforms = student_hidden_transforms.transpose(0,2)
                student_hidden_transforms = student_hidden_transforms.transpose(1,2)

                student_hidden_transforms,teacher_hidden_transforms = self.transform_match(\
                                    student_hidden_transforms,teacher_hidden_transforms,transformList,batch)
                delta_hidden = (teacher_hidden_transforms - self.distillation_out(self.distillation_in(student_hidden_transforms))) ** 2
                delta_hidden = delta_hidden * (1- batch.tgt_len_mask).float().cuda().unsqueeze(2)


                delta_hidden = 1/2 * torch.sum(delta_hidden,dim=[1,2])

        
        gen_from_vocab_prob = F.softmax(out_vocab_vectors, dim=-1)
        ...
        if self.args.no_copy:
            ...
            out_prob = [gen_from_vocab_prob * batch.gen_token_mask.unsqueeze(2) , apply_rule_prob * batch.apply_rule_mask.unsqueeze(2) ]
        else:
            ...
            out_prob = [gen_from_vocab_prob * (batch.gen_token_mask.unsqueeze(2)) * primitive_predictor[:, :, 0].unsqueeze(2),\
                        apply_rule_prob * batch.apply_rule_mask.unsqueeze(2),\
                        primitive_copy_prob * batch.primitive_copy_mask.unsqueeze(2) *  primitive_predictor[:, :, 1].unsqueeze(2)
                       ]
            # avoid nan in log
        ...
        if self.args.func == "KL" and transformList is not None:
            #print("action_prob:",action_prob.shape)
            currrent_prob = out_prob
            delta_hidden = [0,0]
            
            for i in range(len(out_prob)):
                #break
                currrent_prob[i] ,teacher_hidden[i] = self.transform_match(out_prob[i],teacher_hidden[i],transformList,batch)
                currrent_prob[i] = currrent_prob[i].masked_fill_(currrent_prob[i] == 0,1.e-7)
                teacher_hidden[i] = teacher_hidden[i].masked_fill_(teacher_hidden[i] == 0,1.e-7)
                
                
                temp = self.criterion_kl_loss(currrent_prob[i].transpose(0,2).transpose(1,2).log(),
                                                      teacher_hidden[i].transpose(0,2).transpose(1,2))
                temp = temp.masked_fill_(temp == np.inf, 0)
                delta_hidden[0] += torch.sum(temp,dim=[1,2])
                #print("delta_hidden:",delta_hidden[0] )
            
            
                temp = self.criterion_kl_loss(teacher_hidden[i].transpose(0,2).transpose(1,2).log(),
                                                  currrent_prob[i].transpose(0,2).transpose(1,2))
                temp = temp.masked_fill_(temp == np.inf, 0)
                delta_hidden[1] += torch.sum(temp,dim=[1,2])
                #/ torch.FloatTensor(batch.tgt_lens).cuda()
        ...
        return returns, delta_hidden,torch.FloatTensor(batch.tgt_lens).cuda()
    ...
    def parse(self, src_sent, context=None, beam_size=5, debug=False):
        ...
        t = 0
        hypotheses = [DecodeHypothesis(level_traversal=self.level_traversal)]
        hyp_states = [[]]
    ...
    def load(cls, model_path, cuda=False):
        ...
        parser = cls(saved_args, vocab, transition_system,init=False)
