...
@Registrable.register('default_teacher_parser')
class ParserTeacher(nn.Module):
    ...
    def __init__(self, args, vocab, transition_system,init=True,srcEmbed=None,srcLstm=None):
        super(ParserTeacher, self).__init__()
        self.args = args
        self.vocab = vocab
        self.level_traversal= True
    ...
    def score(self, examples, return_encode_state=False):
        ...
        if self.args.func == "OLSE":
            return returns, query_vectors  
        else :
            return returns, out_prob
    ...
    def parse(self, src_sent, context=None, beam_size=5, debug=False):
        ...
        t = 0
        hypotheses = [DecodeHypothesis(level_traversal=self.level_traversal,right_left=self.right_left)]
        hyp_states = [[]]
