...
src_dropout=0.3
lambda_num=0.5
model_name=model.ifttt.seed${seed}.mutual.share.encoder.dropout${dropout}.src_dropout${src_dropout}.lambda0.0.multitask
...
python -u exp_distillation_share_embedding.py \
    ...
    --pretrain ${model_name} \
    --pretrain_teacher saved_models/ifttt/${model_name}.teacher.bin \
    --pretrain_student saved_models/ifttt/${model_name}.student.bin \
    --vocab ${vocab} \
    --lstm ${lstm} \
    ...
    --beam_size ${beam_size} \
    --log_every 50 \
    --save_to saved_models/ifttt/${model_name}.lambda${lambda_num} 2>&1 | tee -a logs/ifttt/${model_name}.lambda${lambda_num}.log
