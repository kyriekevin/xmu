...
train_teacher_file="data/atis/train_r2l.bin"
...
lambda_num=0
max_epoch=60
model_name=model.atis.seed${seed}.share.encoder.dropout${dropout}.src_dropout${src_dropout}.multitask.r2l

echo "**** Writing results to logs/atis/${model_name}.log ****"
mkdir -p logs/atis
echo commit hash: `git rev-parse HEAD` > logs/atis/${model_name}.log

python -u exp_distillation_share_encoder_multitask.py \
    --cuda \
    --seed ${seed} \
    --mode train \
    --batch_size 10 \
