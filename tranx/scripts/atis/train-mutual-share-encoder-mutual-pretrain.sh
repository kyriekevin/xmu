...
lambda_num=0.0
max_epoch=80
model_name_load=model.atis.seed${seed}.mutual.share.encoder.dropout${dropout}.src_dropout${src_dropout}.multitask.pretrain
model_name_out=model.atis.seed${seed}.mutual.share.encoder.dropout${dropout}.src_dropout${src_dropout}.mutual.lambda${lambda_num}.with_pretrian

echo "**** Writing results to logs/atis/${model_name_out}.log ****"
mkdir -p logs/atis
echo commit hash: `git rev-parse HEAD` > logs/atis/${model_name_out}.log
...
python -u exp_distillation_share_encoder.py \
    ...
    --pretrain_student saved_models/atis/${model_name_load}.student.bin \
    --pretrain_teacher saved_models/atis/${model_name_load}.teacher.bin \
    ...
    --log_every 50 \
    --save_to saved_models/atis/${model_name_out} 2>&1 | tee -a logs/atis/${model_name_out}.log
