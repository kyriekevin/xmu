...
train_student_file="data/geo/train_teacher.bin"
...
src_dropout=0.5
lambda_num=0.25
model_name=model.geo.seed${seed}.dropout${dropout}.src_dropout${src_dropout}.no.share.lambda${lambda_num}.${traverse_first}

echo "**** Writing results to logs/geo/${model_name}.log ****"
mkdir -p logs/geo
echo commit hash: `git rev-parse HEAD` > logs/geo/${model_name}.log

python -u exp_distillation_same_model.py \
    --cuda \
    --seed ${seed} \
    --mode train \
    --batch_size 10 \
