...
class Beam:
    def __init__(self, size, pad, bos, eos,
        ...
        self.eos_counters = torch.zeros(size, dtype=torch.long).to(
            self.next_ys[0].device)  # Store the number of emitted eos token for each hypothesis sequence
    ...
    def advance(self, word_logits, attn_dist):
        ...
        * `attn_dist`- attention at the last step [beam_size, src_len]
        ...
        self.next_ys.append(
            (best_scores_idx - prev_k * vocab_size))  # convert it to the vocab indices, LongTensor, size: [beam_size]
        self.attn.append(attn_dist.index_select(0,
                                                prev_k))  # select the attention dist from the corresponding beam, size: [beam_size, src_len]
        ...
        for i in range(self.next_ys[-1].size(0)):  # For each generated token in the current step, check if it is EOS
            if self.next_ys[-1][i] == self._eos:
                ...
                if self.eos_counters[
                    i] == 1:  # compute the score penalize by length and coverage amd append add it to finished
        ...
        if minimum is not None:
            ...
            while len(self.finished) < minimum:
                ...
                self.finished.append(
                    (s, len(self.next_ys) - 1, i))  # score, length of sequence (include eos but not bos), beam_idx
                i += 1
