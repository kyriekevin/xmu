...
def train_model(model, model1, optimizer, optimizer1, train_data_loader, valid_data_loader, opt):
    ...
    for epoch in range(opt.start_epoch, opt.epochs + 1):
        ...
        for batch_i, batch in enumerate(train_data_loader):
            ...
            batch_loss_stat = train_one_batch(batch, model, model1, optimizer, optimizer1, opt)
...
def train_one_batch(batch, model, model1, optimizer, optimizer1, opt):
    ...
    result_token = model(src, src_lens, src_oov, max_num_oov, src_mask)
    reward, similarity = model1(src, src_mask, result_token)
    ...
    # softmax
    # similarity = similarity.to(loss.device)
    # loss1.backward(loss1.clone().detach())
